...
public class Table {
	...
	public static Tile getStrongest() {
		Tile strongest = null;
		int maxStrongest = -1;
		...
		for (int i = 0; i < ROWS; i++) {
			for (int j = 0; j < COLUMNS; j++) {
				if (map[i][j] != null) {
					if (map[i][j].isFighter()) {
						if (map[i][j].getHealth() > maxStrongest) {
	...
	public static Tile getWeakest() {
		...
		for (int i = 0; i < ROWS; i++) {
			for (int j = 0; j < COLUMNS; j++) {
				if (map[i][j] != null) {
					if (map[i][j].isFighter()) {
						if (map[i][j].getHealth() < maxWeakest) {
